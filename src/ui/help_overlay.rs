...
pub fn draw_help_overlay(
    ...
    egui::Window::new("Управление  [F1]")
        ...
        .show(ctx, |ui| {
            ...
            section!("Камера");
            row!("Scroll",         "Зум (приближение/удаление)");
            row!("ССК + тащить",   "Вращение орбиты (orbit)");
            row!("Z / C",          "Поворот камеры влево / вправо");
            ...
            row!("W / A / S / D",  "Движение камеры редактора");
            row!("Scroll",         "Зум");
            row!("Z / C",          "Поворот камеры редактора");
            row!("Esc",            "Выйти из редактора");
