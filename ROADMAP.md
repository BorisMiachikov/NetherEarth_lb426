...
**Последнее обновление:** 2026-04-20 — UI начальных ресурсов в редакторе сценариев (чекбокс + 7 полей); при «Открыть» подгружается description/initial_resources из сценария  
...
- [x] **6.13** Стартовые условия из scenario RON: начальные ресурсы, warbase, позиции `[M]`
...
- [x] **10.10** Windows release build: `cargo build --release`, тест на чистой системе `[M]`
...
- [x] **11.20** UI начальных ресурсов сценария: чекбокс + 7 DragValue-полей в правой панели редактора; сериализация в `data/scenarios/*.ron` поле `initial_resources`; подгрузка description/ресурсов из сценария при «Открыть» `[M]`
